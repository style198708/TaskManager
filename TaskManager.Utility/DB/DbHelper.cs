...
namespace TaskManager.Utility
{
    internal static class DbHelper
    {
        ...
        /// 数据库类型
        /// </summary>
        private static string sqltype = "mssql";

        /// <summary>
        /// 数据库链接
        /// </summary>
        private static string strConnect = SysConfig.SqlServerConnect;


        internal static int ExecuteNonQuery(string strCon, string strSQL, object param)
        {
            using (DbConnection con = GetConnection(strCon))
            {
                ...
                GetCommand(ref cmd, strSQL, param);
        ...
        internal static DataTable RunProcedure(string strCon, string procName, object param)
        {
            ...
            {
                DbCommand cmd = con.CreateCommand();
                GetCommand(ref cmd, procName, param);
        ...
        /// <returns>T</returns>
        private static T GetSingleT<T>(DbDataReader dr)
        {
            Type type = typeof(T);
        ...
        /// <returns></returns>
        private static DbConnection GetConnection(string connectionstr)
        {
            DbConnection con = null;
            switch (sqltype)
            {
                case "sqlite": con = new SQLiteConnection(connectionstr); break;
                case "mssql": con = new SqlConnection(connectionstr); break;
                default: con = new SqlConnection(connectionstr); break;
            }
            return con;
        }
        ...
        public static DbDataAdapter GetDbDataAdapter(DbCommand cmd)
        {
            ...
            switch (sqltype)
            {
                case "sqlite": adp = new SQLiteDataAdapter(cmd.CommandText, cmd.Connection.ConnectionString); break;
                case "mssql": adp = new SqlDataAdapter(cmd.CommandText, cmd.Connection.ConnectionString); break;
                default: adp = new SqlDataAdapter(cmd.CommandText, cmd.Connection.ConnectionString); break;
            }
            return adp;
        }
